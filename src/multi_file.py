...
from nifti_simulator import NIfTISimulator
...
class NIfTI_CLI:
    ...
    def _create_parser(self):
        ...
        parser.add_argument("--input_dir", type=str, default="data/", help="Directory with NIfTI files")
        parser.add_argument("--output_dir", type=str, default="outputs/", help="Directory for outputs and JSON if not specified")
        parser.add_argument("--gif_dir", type=str, default="gifs/", help="Directory for GIFs")
        ...
        parser.add_argument("--remove_param", type=float, default=5, help="Number/fraction of slides to remove (missing_slides)")
    ...
    def run(self):
        ...
        nifti_files = [f for f in os.listdir(args.input_dir) if f.endswith(('.nii', '.nii.gz'))]
        if not nifti_files:
        ...
        if "mixed_axis" in args.sim_type:
            if not all(x in [0, 1, 2] for x in args.mixed_axis_list) or len(args.mixed_axis_list) < 1 or len(args.mixed_axis_list) > 3:
        ...
        for nifti_file in nifti_files:
            file_path = os.path.join(args.input_dir, nifti_file)
